...
library(tidyverse)
  ...
  Teams %>% 
    ...
    aux<- lm(estimate ~ yearID, data = reg)
    summary(aux)
    ...
    Teams %>% 
      ...
      geom_line(color = "blue", size = 1) + 
        ...
        geom_point(aes(x = yearID, y = estimate))
    ...
    coefs
          ...
          imagine <- data.frame(teamID = c("Team A", "Team B"),
                                BB = c(2, 1),
                                singles = c(4, 6),
                                doubles = c(1, 2),
                                triples = c(0, 1),
                                HR = c(1, 0))
          R_hat = predict(fit, newdata = imagine)
          imagine %>% 
            mutate(R_hat = predict(fit, newdata = .)) %>%
            ggplot(aes(R_hat, teamID, label = teamID)) + 
            geom_point() +
